...
this_phase = trial_config('Phase');

xc = smart_eval(trial_config('StimCenterX'));
yc = smart_eval(trial_config('StimCenterY'));

LTRB = [500-50 400-50 500+50 400+50];

if strcmpi(trial_config('Stimuli'), 'circle')
    Screen('FrameOval', wind, [], LTRB);
elseif strcmpi(trial_config('Stimuli'), 'circle')
    Screen('FrameRect', wind, [], LTRB);
end
...
key_code = [];
while 1;
    key_code = check_keypress(key_code, this_phase);
    if any(key_code)
        break;
    end
end
key_code = check_keypress(key_code, this_phase, 'end_trial'); 
    % if they are currently holding down a key, send keypress end msg
